...
const normalizeDashboard = (res) => unwrap(res);
...
const normalizeSales = (res) => {
  ...
  return rows.map((row) => ({
    ...
    period_label: row.period_label || (
      row.period_start
        ? new Date(row.period_start).toLocaleDateString('en-BD', { month: 'short', day: 'numeric' })
        : String(row.period || '')
    ),
    order_count: Number(row.order_count || 0),
    revenue: Number(row.revenue || 0),
    avg_order_value:
      row.avg_order_value != null
        ? Number(row.avg_order_value || 0)
        : (Number(row.order_count || 0) > 0 ? Number(row.revenue || 0) / Number(row.order_count || 1) : 0),
  }));
};

const normalizeTopProducts = (res) => {
  const payload = unwrap(res);
  ...
  return rows.map((p) => ({
    ...
    units_sold: Number(p.total_quantity ?? p.units_sold ?? 0),
    order_count: Number(p.order_count ?? 0),
    is_active: p.is_active,
  }));
};

const normalizeStatusBreakdown = (res) => {
  const payload = unwrap(res);
  return Array.isArray(payload?.breakdown) ? payload.breakdown : [];
};

const normalizeLowStock = (res) => {
  const payload = unwrap(res);
  return Array.isArray(payload?.variants) ? payload.variants : [];
};

const normalizeCategoryPerformance = (res) => {
  const payload = unwrap(res);
...
const normalizeReviewStats = (res) => unwrap(res);
...
const AdminAnalytics = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setDashboard(normalizeDashboard(dashRes));
        setStatusBreakdown(normalizeStatusBreakdown(statusRes));
        setCatPerf(normalizeCategoryPerformance(catRes));
        setReviewStats(normalizeReviewStats(reviewRes));
        setLowStock(normalizeLowStock(lowRes));
      } catch (err) {
        if (active) setError(err?.response?.data?.error || err?.error || err?.message || 'Failed to load analytics.');
      } finally {
  ...
  const reloadLowStock = async () => {
    try {
      ...
      setLowStock(normalizeLowStock(res));
    } catch {
      // silent
    }
  };
  ...
  return (
    <div className="an-page">
      ...
      {dashboard && (
        <div className="an-kpi-strip">
          {[
            ...
            {
              label: 'Low Stock',
              value: fmtNum(dashboard.alerts?.low_stock_variants),
              sub: 'Variants 1–5 units',
              accent: Number(dashboard.alerts?.low_stock_variants || 0) > 0 ? '#dc2626' : undefined
            },
          ].map((k) => (
              ...
              {sales.slice(-30).map((row, i) => {
                ...
                return (
                  <div
                    ...
                    title={`${row.period_label}: ${fmt(row.revenue)} · ${row.order_count} orders`}
                  >
                    <div className="an-bar" style={{ height: `${h}%` }} />
                    {i % Math.ceil(sales.length / 8) === 0 && (
                      <span className="an-bar-label">{String(row.period_label || '').slice(0, 6)}</span>
                    )}
                  </div>
                );
              })}
                ...
                return (
                  <div key={row.status} className="an-status-row">
                    ...
                    <div className="an-progress-track">
                      <div className="an-progress-bar" style={{ width: `${pct}%`, background: meta.color }} />
                    </div>
                  </div>
                );
              })}
              ...
              <tbody>
                {topProducts.map((p, i) => (
                  <tr key={p.product_id}>
                    ...
                    <td>
                      <p className="an-product-name">
                        {p.name}
                        {p.is_active === false && <span className="an-inline-muted"> · inactive</span>}
                      </p>
                ...
                {lowStock.map((v) => (
                  <tr key={v.variant_id}>
                    <td>
                      ...
                      <p className="an-product-sku">{v.product_sku || '—'}</p>
                    </td>
                    <td>{v.size || <span className="an-muted">Unsized</span>}</td>
                    <td>
                      <span className="an-stock-badge">
                        {v.stock_quantity}
                      </span>
                    </td>
                  </tr>
          ...
          {!reviewStats ? (
            ...
            <>
              ...
              <div className="an-star-bars">
                {starKeys.map((key, i) => {
                  const count = Number(reviewStats.summary?.[key] || 0);
      ...
      <style>{`
        ...
        .an-inline-muted { color: var(--muted); font-weight: 500; font-size: 12px; }
