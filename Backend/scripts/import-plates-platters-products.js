...
const INPUT_PATH =
  process.argv[2] ||
  path.join(__dirname, 'output', 'aarong_plates_platters.json');
...
const normalizeSku = (value) => {
  ...
  // Keep SKU deterministic while respecting DB varchar(50).
  let hash = 0;
...
async function upsertPlateOrPlatterItem(client, categoryId, item) {
  const productName = safeText(item.product_name);
  const sku = normalizeSku(item.product_code);
...
async function run() {
  ...
  if (!Array.isArray(items) || items.length === 0) {
    throw new Error('Input JSON must be a non-empty array of plates/platters products.');
  }
  ...
  try {
    ...
    for (const item of items) {
      const result = await upsertPlateOrPlatterItem(client, platesPlattersCategoryId, item);
    ...
    const verify = await client.query(
      `SELECT c.name AS category_name, p.product_id, p.name, pv.sku
        ...
        LIMIT 5`,
      [platesPlattersCategoryId]
    );

    console.log('Plates & Platters import completed.');
    console.log(`Category ID: ${platesPlattersCategoryId}`);
    console.log(`Inserted: ${inserted}, Updated: ${updated}, Skipped: ${skipped}`);
    console.log('Latest plates/platters products:');
...
run().catch((error) => {
  console.error('Plates & Platters import failed:', error.message);
  process.exit(1);
});
