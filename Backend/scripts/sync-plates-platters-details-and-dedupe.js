...
const INPUT_PATH = process.argv[2] || path.join(__dirname, 'output', 'aarong_plates_platters.json');
...
async function run() {
  ...
  try {
    ...
    for (const row of rows.rows) {
      ...
      const specs = item?.specifications && typeof item.specifications === 'object' ? item.specifications : {};
      const nextColor = String(specs.colour || 'Mixed').trim().slice(0, 20) || 'Mixed';
    ...
    console.log('Plates & Platters detail sync and dedupe completed.');
    console.log(`Products updated from JSON: ${detailUpdates}`);
    console.log(`Products deleted as duplicates: ${deletedProducts}`);
    console.log(`Final plates/platters product count: ${final.rows[0].total}`);
  } catch (error) {
...
run().catch((error) => {
  console.error('Plates & Platters sync/dedupe failed:', error.message);
  process.exit(1);
});
